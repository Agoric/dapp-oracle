...
/**
 * @param {{ http: any, board: any, feeIssuer: Issuer, invitationIssuer: Issuer }} param0
 */
const startSpawn = async (
  { http, board, feeIssuer, invitationIssuer },
  _invitationMaker,
) => {
  ...
  /** @type {OracleHandler} */
  const oracleHandler = {
    async onQuery(_query) {
      return {
        calculateDeposit(_q) {
          return {
            // Fee: feeAmountMath.make(2),
          };
        },
        calculateFee(_q) {
          return {
            Fee: feeAmountMath.make(1),
          };
        },
        getReply(q) {
          return q * 2;
        },
        completed(_q, _reply, collected) {
              ...
              async onMessage(obj, { _channelHandle }) {
                ...
                switch (obj.type) {
                  ...
                  case 'encouragement/sendInvitation': {
                    ...
                    const invitationHandleBoardId = await E(board).getId(
                      handle,
                    );
